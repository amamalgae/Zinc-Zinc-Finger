...
const en = {
  ...
  genomeHint: "FASTA / FASTA.gz / ZIP of FASTA files · processed locally",
  genomeClear: "Remove",
  genomeChecking: "Checking exact pairs…",
  genomeReady: (files: number, sequences: number, bases: number) => `${files} FASTA · ${sequences} sequences · ${bases.toLocaleString()} bp`,
  genomeScope: "Exact pair = both complete half-sites at the same spacer length; spacer bases are ignored. This is not an off-target prediction.",
  genomeUnique: "genome unique",
  genomeNotFound: "genome not found",
  genomeRepeated: (count: number) => `genome ×${count}`,
...
const ja: Copy = {
  ...
  resetExample: "例に戻す",
  genomeLabel: "ゲノム（任意）",
  genomeHint: "FASTA / FASTA.gz / FASTA を含む ZIP · 端末内で処理",
  genomeClear: "解除",
  genomeChecking: "完全一致を確認中…",
  genomeReady: (files: number, sequences: number, bases: number) => `${files} FASTA · ${sequences} 配列 · ${bases.toLocaleString()} bp`,
  genomeScope: "完全一致 = 左右の half-site 全体と同じ spacer 長。spacer の塩基配列は無視します。off-target 予測ではありません。",
  genomeUnique: "ゲノム 一意",
  genomeNotFound: "ゲノム 未検出",
  genomeRepeated: (count: number) => `ゲノム ×${count}`,
