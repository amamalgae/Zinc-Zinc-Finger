...
const en = {
  ...
  genomeScope: "Similarity check: both complete half-sites, both genomic orientations, 5–7 bp spacers, ≤4 mismatches per half-site and ≤5 total. Spacer bases are ignored. This is a sequence-similarity guardrail, not an off-target prediction.",
  ...
  genomeNoClose: "no alternate ≤5 mm",
  genomeMismatchCounts: (counts: number[]) => `1–5 mm: ${counts.join("/")}`,
...
const ja: Copy = {
  ...
  genomeScope: "類似配列チェック：左右の half-site 全体、両方向、spacer 5–7 bp、各 half-site 4 mismatch 以下かつ合計 5 mismatch 以下を検索します。spacer の塩基配列は無視します。off-target 予測ではなく配列類似性の回避補助です。",
  ...
  genomeNoClose: "別候補 ≤5 mm なし",
  genomeMismatchCounts: (counts: number[]) => `1–5 mm: ${counts.join("/")}`,
  genomeRankingActive: "ゲノム順位補正を有効化しています。別locusの完全一致は強く降格し、それ以外は設計法の機能的根拠を優先します。1–2 mismatch は強めの secondary factor、3–4 mismatch は弱い tie-break、5 mismatch は順位に使いません。",
