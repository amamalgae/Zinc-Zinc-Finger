...
Last reconciled: 2026-08-17, for PR #28 based on `main` commit `3a9cc34`, plus the source files listed in section 10.
...
12. **Explain the selected molecular geometry with the selected sequence.** The selected-pair section must map the actual F/R duplex to six global display labels: left monomer ZF1–ZF3 and right monomer ZF4–ZF6. Along F from left to right, the labels are ZF1–ZF2–ZF3, spacer, ZF6–ZF5–ZF4 because C2H2 arrays bind antiparallel. The protein lanes must show N→C and place both C-terminal FokI domains toward the spacer. Use plus/minus only for the FokI dimerization interfaces—ELD (−) and KKR (+)—not as DNA strand names. The global ZF1–ZF6 labels are explanatory; archive logic still calls each monomer's units F1–F3.
13. **Teach the ZFN architecture before asking for sequence.** The overview between hero and input is a site-native inline SVG, not a copied paper or slide image. It must show exactly three fingers per monomer, F/R DNA labels, two 9-bp recognition regions, a 5–7 bp spacer, antiparallel N→C directions, and FokI ELD (−)/KKR (+) meeting at the spacer. It should separate the roles as “ZF recognizes” and “FokI cuts,” then lead directly into the designer. The selected-candidate diagram remains the detailed sequence-specific view.
...
### 7.2 Complete main-branch commit/PR ledger through PR #28
...
| 2026-08-17 | PR #28 | Added an original pre-input SVG overview of the two 3ZF monomers, F/R duplex, 5–7 bp spacer, and FokI heterodimer mechanism. |
