...
12. **The public interface is written in English.** Every string the site renders — hero, diagram, the three steps, evidence, validation messages, disclosure labels, the donor list in `src/zfn-construct-output.ts`, the `content:` labels in CSS, and the `index.html` description and `lang` — is English. `tests/public-evidence.test.mjs` fails on any kana or kanji in those surfaces. Legacy research modules that the public UI does not render (`off-target-engine.ts`, `off-target.worker.ts`, `fauser-context.ts`, `construct-output.ts`, `coda-construct-output.ts`) keep their Japanese strings; translate them only if they are ever surfaced.
13. **The three steps are minimal, and prose is not how they explain themselves.** Prose instructions were tried and removed: with a paragraph under every heading the panels lost their rhythm and the reader could no longer tell where to look. `01 INPUT / Enter the target`, `02 SELECT / Pick a candidate`, `03 PROTEIN OUTPUT / Get the sequence` are parallel and short; the numbered badge, the section key and the controls carry the meaning. Field names inside the panels are single English words: `Target DNA`, `Method`, `Spacer center`, `Range ±bp`. Status text appears only when there is something to say: the input meta row shows the length always, and ambiguous-base or unsupported-character counts only when non-zero. Do not reintroduce per-step explanatory paragraphs; if a control needs an explanation, the control is wrong.
